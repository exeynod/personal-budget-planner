...
async function mockApiRich(page: import("@playwright/test").Page) {
  // This spec asserts the v06 BottomNav (button[aria-label="Главная"] …). The
  // app default shell is now v10 (componentsV10/TabBar), so pin the shell to
  // v06 before the app boots, otherwise the v10 TabBar renders and the v06
  // selectors never appear. Also pre-acknowledge the cookie banner — it renders
  // on first visit (no `cookie_consent_v1` in localStorage) and its fixed
  // region intercepts pointer events on the bottom nav. Must run before goto().
...
// ============================================================
// NAV-01: 4 функциональных таба + central FAB (post-ui-glass)
// ============================================================
test("nav-01: 4 таба + central FAB с новыми лейблами", async ({ page }) => {
  await mockApiRich(page);
  await page.goto("/");
  await waitForLoad(page);

  // Текущий nav: 4 tab-кнопки. Аналитика ушла в Management hub.
  await expect(page.locator('button[aria-label="Главная"]')).toBeVisible();
  ...
  // Central FAB — context-aware action, default = «Добавить транзакцию».
  await expect(
    page.locator('button[aria-label="Добавить транзакцию"]'),
  ).toBeVisible();

  // Аналитика — больше не tab (sub-screen в Management).
  await expect(
    page.locator('button[aria-label="Аналитика"]'),
  ).not.toBeVisible();
...
test("txn-02: История группирует по дням с day-total", async ({ page }) => {
  ...
  const dayHeader = page
    .locator('[class*="dayHeader"], [class*="day-header"]')
    .first();
...
test("txn-03: Под-таб план показывает строки плана", async ({ page }) => {
  ...
  const planAmount = page
    .locator('[class*="amount"]', { hasText: /\d.*₽/ })
    .first();
...
// ============================================================
test("txn-05: FAB в История открывает форму факт-транзакции", async ({
  page,
}) => {
  await mockApiRich(page);
  await page.goto("/");
  ...
  // BottomSheet с "Новая транзакция" должен открыться. В DOM — несколько
  // dialog'ов с тем же aria-label (App-level + HistoryView'ный), берём
  // именно открытый — у его контейнера дополнительный класс sheetOpen
  // (BottomSheet.module.css).
  await expect(
    page.locator(
      'div[role="dialog"][aria-label="Новая транзакция"][class*="sheetOpen"]',
    ),
  ).toBeVisible({ timeout: 5000 });
});

// ============================================================
// MGT-01: Таб Управление показывает пункты меню
// ============================================================
test("mgt-01: Управление показывает пункты меню", async ({ page }) => {
  await mockApiRich(page);
  await page.goto("/");
  await waitForLoad(page);
  ...
  // Пункты меню — кнопки-ряды (role=button с aria-label = label + description).
  // Используем role=button чтобы не цепляться за subtitle "Подписки, категории, доступ".
  await expect(page.getByRole("button", { name: /^Подписки\s/ })).toBeVisible();
  await expect(
    page.getByRole("button", { name: /^Шаблон бюджета\s/ }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: /^Категории\s/ }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: /^Настройки\s/ }),
  ).toBeVisible();
});
...
// ============================================================
test("mgt-02: Клик Подписки открывает SubscriptionsScreen", async ({
  page,
}) => {
  await mockApiRich(page);
  await page.goto("/");
  ...
  // Click Subscriptions row in management menu (а не subtitle).
  await page.getByRole("button", { name: /^Подписки\s/ }).click();
  await page.waitForTimeout(400);
  ...
  await expect(page.locator("text=Netflix").first()).toBeVisible({
    timeout: 5000,
  });
});
...
test.skip("placeholder: Аналитика и AI показывают Скоро будет", async ({
  page,
}) => {
  await mockApiRich(page);
  await page.goto("/");
