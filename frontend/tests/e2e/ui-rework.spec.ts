      ...
      return route.fulfill({
        ...
        body: JSON.stringify([
          ...
          },
          {
            ...
            starting_balance_cents: 4800000,
            ending_balance_cents: 5000000,
            status: "closed",
    ...
    if (url.includes("/api/v1/admin/users")) {
      return route.fulfill({
        ...
        body: JSON.stringify([
          ...
          {
            id: 2,
            tg_user_id: 234567890,
            tg_chat_id: null,
            role: "member",
            last_seen_at: "2026-05-07T15:30:00+00:00",
          ...
          {
            id: 3,
            tg_user_id: 345678901,
            tg_chat_id: null,
            role: "revoked",
            last_seen_at: "2026-04-20T08:00:00+00:00",
            onboarded_at: null,
...
test("rework-01: Home — expenses tab", async ({ page }) => {
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/01-home-expenses.png`,
    fullPage: true,
  });
});
...
test("rework-02: Home — income tab", async ({ page }) => {
  ...
  await page.waitForTimeout(300);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/02-home-income.png`,
    fullPage: true,
  });
});

test("rework-03: Home — empty state", async ({ page }) => {
  // Inline mock (not via mockApiRich), so pin the v06 shell + dismiss the
  // cookie banner here too — otherwise the v10 TabBar renders and the v06
  // BottomNav selector in waitForLoad never appears.
  await page.addInitScript(() => {
    try {
      window.localStorage.setItem("ui.shell", "v06");
      window.localStorage.setItem("cookie_consent_v1", "acknowledged");
    ...
    if (url.includes("/api/v1/me"))
      return route.fulfill({
        ...
        body: JSON.stringify({
          ...
          tg_chat_id: null,
          cycle_start_day: 5,
          onboarded_at: "2026-04-05T00:00:00Z",
          chat_id_known: false,
          role: "owner",
          ai_spend_cents: 0,
          ai_spending_cap_cents: 46500,
  ...
  await page.waitForTimeout(300);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/03-home-empty.png`,
    fullPage: true,
  });
});
...
test("rework-04: Transactions — history (FAB visible)", async ({ page }) => {
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/04-transactions-history.png`,
    fullPage: true,
  });
});
...
test("rework-05: Transactions — Plan sub-tab", async ({ page }) => {
  ...
  await page.waitForTimeout(400);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/05-transactions-plan.png`,
    fullPage: true,
  });
});

test("rework-06: Transactions — history filtered by category", async ({
  page,
}) => {
  await mockApiRich(page);
  await page.goto("/");
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/06-transactions-history-filtered.png`,
    fullPage: true,
  });
});
...
test("rework-07: Analytics", async ({ page }) => {
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/07-analytics.png`,
    fullPage: true,
  });
});
...
test("rework-08: AI — empty", async ({ page }) => {
  ...
  await page.waitForTimeout(400);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/08-ai-empty.png`,
    fullPage: true,
  });
});
...
test("rework-09: Management — hub", async ({ page }) => {
  ...
  await page.waitForTimeout(400);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/09-management-hub.png`,
    fullPage: true,
  });
});
...
test("rework-10: Onboarding", async ({ page }) => {
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/10-onboarding.png`,
    fullPage: true,
  });
});
...
test("rework-11: Management — Subscriptions list", async ({ page }) => {
  ...
  await page.waitForTimeout(400);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/11-management-subscriptions.png`,
    fullPage: true,
  });
});
...
test("rework-12: Management — Template list", async ({ page }) => {
  ...
  await page.waitForTimeout(400);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/12-management-template.png`,
    fullPage: true,
  });
});
...
test("rework-13: Management — Categories list", async ({ page }) => {
  ...
  await page.waitForTimeout(400);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/13-management-categories.png`,
    fullPage: true,
  });
});
...
test("rework-14: Management — Settings", async ({ page }) => {
  ...
  await page
    .locator("button")
    .filter({ hasText: /Настройки/ })
    .first()
    .click();
  await page.waitForTimeout(400);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/14-management-settings.png`,
    fullPage: true,
  });
});
...
test("rework-15: Management — Access (owner-only)", async ({ page }) => {
  ...
  await page.waitForTimeout(500);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/15-management-access.png`,
    fullPage: true,
  });
});
...
test("rework-16: Add transaction bottom-sheet (FAB on Transactions)", async ({
  page,
}) => {
  await mockApiRich(page);
  await page.goto("/");
  ...
  await page.waitForTimeout(300);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/16-add-transaction-sheet.png`,
    fullPage: true,
  });
});
...
test("rework-17: Edit subscription sheet", async ({ page }) => {
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/17-subscription-edit.png`,
    fullPage: true,
  });
});
...
test("rework-18: Plan create sheet (FAB on Plan sub-tab)", async ({ page }) => {
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/18-plan-create-sheet.png`,
    fullPage: true,
  });
});
