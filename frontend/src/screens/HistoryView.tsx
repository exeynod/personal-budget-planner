...
function groupByDate(rows: ActualRead[]) {
  ...
  return [...map.entries()].sort((a, b) => b[0].localeCompare(a[0])).map(([date, rs]) => {
    const sortedRows = rs.sort((a, b) => b.id - a.id);
    const totalCents = sortedRows.reduce(
      (sum, r) => (r.kind === 'expense' ? sum + r.amount_cents : sum),
      0,
    );
    return {
      date,
...
export const HistoryView = forwardRef<HistoryViewHandle, HistoryViewProps>(
  function HistoryView({ onBack, categoryFilter, onClearFilter, inTransactions, activeKindFilter }, ref) {
    ...
    let filteredRows = categoryFilter != null
      ? rows.filter((r) => r.category_id === categoryFilter)
      : rows;

    // Apply kind filter
    if (activeKindFilter && activeKindFilter !== 'all') {
      filteredRows = filteredRows.filter((r) => r.kind === activeKindFilter);
    }
        ...
        {groups.map((g) => (
          <div key={g.date} className={`${styles.dateGroup} ${styles.dayHeader}`}>
            <div className={styles.dateLabel}>
              <span>{g.label}</span>
              {g.totalCents > 0 && (
                <span className={styles.dateTotal}>−{(g.totalCents / 100).toLocaleString('ru-RU')} ₽</span>
              )}
            </div>
            {g.rows.map((row) => {
