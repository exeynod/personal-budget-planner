...
// fields, clamps step ∈ 1..4 (out-of-range → reject entire payload),
// drops unknown category_plans codes, validates account shapes,
...
function sanitiseAccount(raw: unknown): OnboardingAccount | null {
  ...
  if (typeof kind !== 'string' || !ACCOUNT_KINDS.has(kind as AccountKind))
    return null;
...
function sanitiseDraft(raw: unknown): OnboardingDraft | null {
  ...
  // step: must be int in 1..4; out-of-range rejects the whole payload.
  const step = raw.step;
  if (
    typeof step !== 'number' ||
