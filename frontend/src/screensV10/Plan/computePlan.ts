...
//   computeRolloverAggregates(cats, plans, actuals) → {miscCents, savingsCents}
...
//   - T-26-04-05: rollover bucket is type-narrowed to 'misc'|'savings' literal —
//                 unrecognised policies fall through to the misc bucket.
//
...
export interface RolloverAggregates {
  /** Σ remainders for categories with rollover='misc'. */
  miscCents: number;
  /** Σ remainders for categories with rollover='savings'. */
  savingsCents: number;
}

/**
 * Aggregate потенциальных rollover'ов: для каждой категории где fact ≤ plan,
 * добавить (plan − fact) в bucket по rollover policy.
 *
 * Skip rules (mirror computeCategoryAggregates / computePlanTotalCents):
 *   - skip `code === 'savings'` (system category)
 *   - skip `paused === true`
 *   - skip when remainder ≤ 0 (over-budget rows contribute 0)
 *
 * Plans-array takes precedence over `category.plan_cents` so the running
 * draft (slider edits) is reflected live; falls back to category's persisted
 * plan when no draft exists for that category.
 *
 * Fact = Σ |amount_cents| for actuals where kind='expense'; matches
 * Home/CategoryDetail compute so dashboard / detail / plan agree.
 */
export function computeRolloverAggregates(
  ...
  for (const a of actuals) {
    ...
    factByCat.set(
      a.category_id,
      (factByCat.get(a.category_id) ?? 0) + Math.abs(a.amount_cents),
    );
  }
 ...
 * Skips system 'savings' + paused categories (mirror of compute filter).
 *
