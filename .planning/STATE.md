...
current_phase: 71
status: in_progress
stopped_at: "Phase 71 (UI/UX & functional polish, owner /goal) — 23 issues closed, actionable backlog exhausted; product functional + polished on both iOS shells + web. Final iteration: ADDSHEET-INCOME(4f208b7) MP AddSheet hardcoded kind:expense → Доход/Расход toggle (income was impossible); P2-SIGN(099e172) MP expenses showed '+' → kind-driven '−'/'+'; P3-TITLE shrink-to-fit; P3-STEPPER PosterStepper; P3-STATUSBAR(157842c) root-VC swizzle (cream→dark clock). All VERIFIED on sim. empty-desc decided leave per-shell. iOS 686 / backend 787 / web 755 green. Remaining = subjective pixel-perfect vs web-prototype only (no prototype render available; both shells defect-free per scrutiny). ||| EARLIER: 17 issues closed (actionable backlog exhausted). Added: ACCESS-1(c110e8f) v06 Доступ implemented+VERIFIED (reuses AccessV10ViewModel + shared AccessFormatting R6); web P3-W1(1474c83) subscription account UI; web P3-W2(1e7a620) analytics month-chip drives Top-5. iOS 669 / backend 787 / web 755 green. Remaining = only by-design (empty-desc per-shell, AddSheet income-toggle) + subjective pixel-perfect. ||| EARLIER: 14 issues closed (3×P0, 5×P1, 2×P2 fixed +1 deferred, UI/P3). Added since: BAL-1(fe02053) v06 «Остаток на счёте»=сумма счетов VERIFIED; WEB-AI-1(77e69ef) web 402→Pro msg; P3 subtitle-clip(494b9e6)+onboarding-slider-readout(a8e3e1d). Web regression green (build+typecheck+vitest 742). Leftover dev-data cleaned. iOS 663 / backend 787 / web 742 green. Both iOS shells + web reviewed & polished. DEFERRED: ACCESS-1 (v06 Доступ feature), web P3 (sub account_id UI, analytics month-chip), borderline-P3, pixel-perfect. Owner=Pro(dev) for AI. Setup: docker dev stack (api :8000), iOS на iPhone 17 Pro sim autologin OWNER (выдан Pro в dev-БД для AI), backend засеян. DEV_FORCE_ONBOARDING снят. NB: full pytest стирает shared dev-БД → пересевать + заново Pro. FIXED+VERIFIED: BUG-1(e21aba2) balance 500→Home; BUG-2(b536653) subs V10 write account+day; HOME-1(8e18b9f P0) compute_balance из Category.plan_cents; theme→2 опции(e96affa); theme-sheet layout(b7c1d84); AI-CHAT-1(e11990f P0) SSE URL malformed (chat был полностью сломан); AI-CHAT-2(4992fbb+e0d0207) 402→Pro-сообщение; PLAN-1(edb09d2) plan-месяца копейки→рубли; DEP-1(2361027) deposit-sheet кнопки за таб-баром; AN-1(3583d56) v06 analytics DTO drift. Core add-flow + оба шелла отревьюены (Maximal Poster + СТАРЫЙ IOS) — визуально вылизаны. iOS suite 663 green, backend 787 green. DEFERRED P2 (нужны решения): ACCESS-1 (v06 Доступ placeholder), BAL-1 (Остаток label vs account total). BACKLOG: web (не тронут), pixel-perfect, P3-полировка, leftover-data cleanup. См. 71-FINDINGS.md. ||| PREVIOUS: Phase 70 COMPLETE (5 plans + WR-01/02 codegen-BusinessDate fix 94e0b9a) — 4/4 PASS; iOS 639 green; both shells build; APIClientForbiddenTests unmodified; BusinessDate + injectable error-policy + shared SubscriptionsDomain/Store; sync-guard green. ENTIRE CONVERGENCE-AND-DEBT-PLAN (68 tech-debt -> 69 codegen R4 -> 70 convergence R3/R6/R7) DONE. Next: UI/UX review loop (owner /goal). ||| Phase 70-05 (D/R6 shared SubscriptionsStore) COMPLETE — FINAL plan of Phase 70. Extracted the duplicated Subscriptions load+mutation logic + the injectable network API seam into a shared @Observable SubscriptionsStore (ios/BudgetPlanner/Domain/Subscriptions/SubscriptionsStore.swift) consumed by BOTH shells; the v06 SubscriptionsViewModel + V10 SubscriptionsV10ViewModel became thin adapters holding ONLY shell-specific presentation (v06 maps store Bool outcomes to fixed-RU mutationError banner / V10 keeps menuSub/pendingDeleteSub/toastMessage). v06 superset ported VERBATIM into the store: submitting-guard (T-63-01), reload-on-success (T-63-04), WR-06 stale-4xx reload on post/unpost (delete/patch no-reload-on-failure kept EXACTLY), reloadPending re-entrancy (WR-01). Store init(api:loadsCategoriesAccounts:sort:) — loadsCategoriesAccounts gates cats/accounts fetch + notification reschedule (v06 true / V10 false); injectable sort closure keeps display order byte-identical (v06=sortV06 stored sorted / V10=identity + own sortedSubs derived getter via sortV10). API seam = v06 WR-04 closure struct lifted to domain layer (static .live proxies SubscriptionsV10API/Categories/Accounts/LocalNotifications). V10 reloadPending-SAFETY VERIFIED: old V10 load() dropped a concurrent load (if inFlight { return }); the superset instead remembers+re-runs it — strictly safer, no V10 caller depends on the drop, no V10 unit test asserted it (V10 VM was never unit-tested). SubscriptionsViewModelTests (19) ran UNMODIFIED (git 0 changes) + green via API/Status typealias + delegated _setStateForTesting backdoor (added DEBUG backdoor to the store). 14 new SubscriptionsStoreTests (seam-injected: v06+V10 load paths, all mutations, submitting-guard, WR-06, WR-01 coalesce). Both shells BUILD SUCCEEDED; full iOS suite 639 green (625 baseline + 14). Minor deviation: V10 failure-toast detail simplified to fixed RU copy (store reports Bool only; matches plan interfaces + T-70-05-03 no-leak; toast still shows on every failure; errMessage helper removed; no test affected). R6 STORE-EXTRACTION PATTERN PROVEN — Subscriptions fully on shared layer (compute via 70-04 + store via 70-05), Views/presentation per-shell; Savings = next D backlog domain. Commits f8a0522 + f0f45d0. Phase 70 (R3/R6/R7, all 5 plans) COMPLETE.
