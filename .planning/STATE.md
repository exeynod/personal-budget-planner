...
current_phase: 71
status: completed
stopped_at: "v1.1.2 followup COMPLETE & GSD closed (2026-05-21). Phases 68→71 all SHIPPED on branch v1.0-maximal-poster (NOT yet merged to master). 99 commits. 68 Tech-Debt (backend pytest 62fail/64err→0, RLS-guard 14 tables, seed-helper). 69 Codegen R4 (openapi→TS+Swift DTO, sync-guard). 70 Convergence R3/R6/R7 (legacy deprecate+registry, BusinessDate, injectable error-policy, shared SubscriptionsDomain/Store). 71 UI/UX & functional polish: 23 issues closed (3×P0: AI-chat broken SSE URL, Home balance 500, ПЛАН=0 core-value; 5×P1; income-toggle; ACCESS-1 v06 Доступ; themes→2; all pixel-polish), всё verified on sim/tests. FINAL TESTS: iOS 686 / backend 787 / web 755 green. Details in per-phase SUMMARY/VERIFICATION + 71-FINDINGS.md. Remaining (not blocking): subjective pixel-perfect vs web-prototype; merge branch→master when ready; 5 pending HUMAN-UAT live-smoke (62/63/64/66/67). Dev env (docker :8000, seeded DB, owner Pro) torn down at cleanup — restart: docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d."
