...
for (const [k, v] of entries(t.color))
  cssLines.push(`  --poster-${kebab(k)}: ${v};`);
for (const [k, v] of entries(t.spacing))
  cssLines.push(`  --poster-space-${k.replace(/^s/, "")}: ${v}px;`);
for (const [k, v] of entries(t.radius))
  cssLines.push(`  --poster-radius-${kebab(k)}: ${v}px;`);
for (const [k, v] of entries(t.shadow))
  cssLines.push(`  --poster-shadow-${kebab(k)}: ${v};`);
for (const [k, v] of entries(t.font))
  cssLines.push(`  --poster-font-${kebab(k)}: '${v}';`);
for (const [k, v] of entries(t.fontSize))
  cssLines.push(`  --poster-font-size-${kebab(k)}: ${v}px;`);
for (const [k, v] of entries(t.letterSpacing))
  cssLines.push(`  --poster-tracking-${kebab(k)}: ${v};`);
for (const [k, v] of entries(t.easing))
  cssLines.push(`  --poster-easing-${kebab(k)}: ${v};`);
cssLines.push("}", "");
...
const emitThemeBlock = (
  themeKey: "maximal_poster" | "liquid_glass" | "ios_default",
) => {
  const theme = t.themes?.[themeKey];
  if (!theme) return;
  const prefix = themePrefix[themeKey];
  const lines: string[] = [];
  for (const [k, v] of entries(theme.color ?? {}))
    lines.push(`  --${prefix}-${kebab(k)}: ${v};`);
  for (const [k, v] of entries(theme.font ?? {}))
    lines.push(`  --${prefix}-font-${kebab(k)}: ${v};`);
  for (const [k, v] of entries(theme.shadow ?? {}))
    lines.push(`  --${prefix}-shadow-${kebab(k)}: ${v};`);
  for (const [k, v] of entries(theme.material ?? {}))
    lines.push(`  --${prefix}-material-${kebab(k)}: ${v};`);
  for (const [k, v] of entries(theme.fontSize ?? {}))
    lines.push(`  --${prefix}-font-size-${kebab(k)}: ${v}px;`);
  for (const [k, v] of entries(theme.motion ?? {}))
    lines.push(`  --${prefix}-motion-${kebab(k)}: ${v};`);
  for (const [k, v] of entries(theme.radius ?? {}))
    lines.push(`  --${prefix}-radius-${kebab(k)}: ${v}px;`);
...
const swiftLines: string[] = [
  ...
  "    }",
  "    enum Space {",
  ...entries(t.spacing).map(
    ([k, v]) => `        static let ${k}: CGFloat = ${v}`,
  ),
  "    }",
  "    enum Radius {",
  ...entries(t.radius).map(
    ([k, v]) => `        static let ${k}: CGFloat = ${v}`,
  ),
  "    }",
  "    enum FontSize {",
  ...entries(t.fontSize).map(
    ([k, v]) => `        static let ${k}: CGFloat = ${v}`,
  ),
  "    }",
  "    enum Font {",
  ...entries(t.font).map(([k, v]) => `        static let ${k} = "${v}"`),
  "    }",
  "    enum Easing {",
  ...entries(t.easing).map(([k, v]) => {
    // parse "cubic-bezier(0.22, 0.61, 0.36, 1)" → control points
    const m = v.match(/cubic-bezier\(([^)]+)\)/);
    const [c0x, c0y, c1x, c1y] = (m?.[1] ?? "0,0,1,1")
      .split(",")
      .map((s) => s.trim());
  ...
  "",
  '    /// Любой неизвестный raw (в т.ч. stale persisted "ios_default" из прежних',
  "    /// версий) безопасно резолвится в .maximalPoster.",
