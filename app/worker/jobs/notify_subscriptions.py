...
Behaviour:
    1. Acquire pg_try_advisory_lock(ADVISORY_LOCK_KEY=20250502); bail if False.
    ...
    5. Release advisory lock in finally; close bot session in finally.

Threat mitigations:
    T-06-07: pg_try_advisory_lock prevents concurrent runs (key 20250502).
...
async def notify_subscriptions_job() -> None:
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            # Release advisory lock even if an error occurred.
            if lock_acquired:
                try:
                    await db.execute(
                    ...
                    logger.exception("notify_subscriptions: advisory unlock failed")
