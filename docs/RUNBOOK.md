...
```
  ...
  (TMA)     │   └─/ → SPA static  ├─→ db (postgres16 + pgvector, RLS)  │
            ...
            │                          close_period 00:01 / purge 02:00 │
            └──────────────────────────────────────────────────────────┘
```
...
`worker` — APScheduler с MemoryJobStore (расписание в коде, не персистится). 3 джобы в `app/worker/jobs/`
(ежедневная `charge_subscriptions` удалена по ADR-0007 — регулярные платежи
материализуются при открытии периода в `close_period`):
...
## Сценарий 6b — Миграции: autogen-drift (models ↔ схема)

`alembic check` сравнивает `Base.metadata` (ORM-модели) с head-схемой и
репортит расхождения. Использовать при написании миграции, чтобы убедиться, что
новые объекты согласованы с моделями.

```bash
make up                    # нужен живой PG (dev api-контейнер)
make migrations-check      # alembic check внутри api: список drift-операций
make migrations-check-selftest   # negative-control: доказывает, что детектор краснеет
```

**Важно — это ИНФОРМАЦИОННЫЙ инструмент, НЕ блокирующий гейт** (не в
`verify-all`/`ci-local`). Codebase несёт намеренные model↔schema расхождения,
которые `alembic check` всегда репортит: composite FK на DB-уровне (`fk_*_composite`),
raw-SQL таблица `analytics_event`, часть perf-индексов объявлена только в миграциях
вне ORM. Поэтому вывод нужно читать глазами: ищи СВОИ новые объекты среди шума.
Новая миграция считается согласованной, если её объекты в drift-списке **не**
появляются (т.е. ORM и DDL совпадают). Перевод в блокирующий гейт потребовал бы
baseline-снапшота известного дрейфа (отложено).
