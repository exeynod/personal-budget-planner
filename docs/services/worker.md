...
| `close_period`         | cron 00:01 MSK                    | закрыть истёкший период (с авто-пропуском непроведённых регулярок), создать новый, перенести шаблон + материализовать регулярные платежи | 20250501     |
...
> проведение в факт — по подтверждению пользователя.
...
- `app/worker/jobs/close_period.py` — закрытие/создание периода (`period_for`,
  ...
  периода; в новом периоде материализует регулярные платежи, сдвигая курсор на
