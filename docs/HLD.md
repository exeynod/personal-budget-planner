...
│ description          │                │ interval_months ≥1   │
│ day_of_period NULL   │                │ cycle enum (deprtd)  │
│ sort_order           │                │ next_charge_date     │
└──────────────────────┘                │ category_id FK       │
...
| `POST`   | `/actual`                                    | Body: `{kind, amount_cents, description, category_id, tx_date, account_id?}`. Период определяется по `tx_date` + `cycle_start_day`. При `account_id` к счёту применяется дельта `signed_delta(kind, amount)`.             |
| `PATCH`  | `/actual/{id}`                               | При изменении `tx_date` — пересчёт привязки к периоду. При изменении `amount_cents`/`kind` у строки со счётом — коррекция баланса счёта на `signed_delta(new) − signed_delta(old)`; `account_id` через PATCH не меняется. |
| `DELETE` | `/actual/{id}`                               | Восстанавливает баланс счёта (`−signed_delta` по каждой удаляемой строке); для строк без `account_id` — no-op по балансу.                                                                                                 |

**Конвенция знака (v1.2 balance-fix):** `amount_cents` хранится и передаётся
**положительным** (`gt=0`) для любого `kind`; знак дельты к
`account.balance_cents` выводится из `kind` единственной точкой истины
`app.services.actual.signed_delta`: `income → +amount`,
`expense`/исторические `roundup`/`deposit` → `−amount`. Агрегации
(`compute_balance`, миграции 0032/0038) используют `ABS(amount_cents)` —
устойчивость к редким legacy-строкам с отрицательным хранением (старый
`reconcile_balance`). Историческая порча балансов (до v1.2 трата
_увеличивала_ счёт, DELETE/PATCH баланс не корректировали) исправлена
data-миграцией `0038_recompute_balances`.
...
| Method   | Path                                                        | Описание                                                                                                                                                                                                    |
| -------- | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`    | `/subscriptions`                                            | Список регулярных платежей                                                                                                                                                                                  |
| `POST`   | `/subscriptions`                                            | Body: `{name, amount_cents, interval_months?, cycle?, next_charge_date, category_id, day_of_month?, account_id?, notify_days_before?}` (ADR-0007: `interval_months` — источник частоты; `cycle` deprecated) |
| `PATCH`  | `/subscriptions/{id}`                                       | —                                                                                                                                                                                                           |
| `DELETE` | `/subscriptions/{id}`                                       | hard delete                                                                                                                                                                                                 |
| `GET`    | `/subscriptions/recurring/due`                              | ADR-0007 home-prompt: сегодня/просрочено                                                                                                                                                                    |
| `POST`   | `/subscriptions/recurring/{planned_id}/pay\|skip\|postpone` | Оплачено / Пропустить / Перенести                                                                                                                                                                           |
| `GET`    | `/subscriptions/recurring/cashflow`                         | Прогноз кэшфлоу на ~90 дней                                                                                                                                                                                 |
...
APScheduler (`AsyncIOScheduler`) с **MemoryJobStore** (дефолт; персистентный jobstore не используется — расписание фиксировано в коде через `replace_existing=True`, рестарт контейнера перерегистрирует джобы). Все джобы — в TZ `Europe/Moscow`.
...
| `close_period`         | ежедневно 00:01 | Для активного периода, чей `period_end < today` — авто-пропуск (удаление) непроведённых `PlannedTransaction(subscription_auto)` закрываемого периода, затем закрыть (`status=closed`, `ending_balance` = расчёт). Создать новый активный период (`planned_at = NULL` → триггерит гейт планирования, ADR-0008), развернуть шаблон + материализовать регулярные платежи, сдвигая `next_charge_date` на `interval_months`. |
| `purge_deleted_users`  | ежедневно 02:00 | Найти юзеров с `deleted_at < now() - 30d` и каскадно удалить их данные (CMP-33-02).                                                                                                                                                                                                                                                                                                                                     |
| `heartbeat`            | каждые 5 мин    | Liveness-пинг в лог (D-12); первый запуск — сразу на старте.                                                                                                                                                                                                                                                                                                                                                            |

> **ADR-0008 (месячный гейт):** `budget_period.planned_at TIMESTAMPTZ NULL` =
> «период не спланирован». `create_first_period` (онбординг) ставит `now()`;
> `close_period` оставляет `NULL` у нового периода → `GET /home.needs_planning`
> становится `true` и shell показывает гейт. `POST /periods/{id}/confirm-plan`
> ставит `planned_at = now()` (идемпотентно), снимая гейт.

> **ADR-0007:** джоба `charge_subscriptions` (00:05) **удалена**. Регулярные
> платежи не списываются по дате автоматически — материализуются как прогнозные
> `PlannedTransaction(subscription_auto)` при открытии периода (`close_period`),
> курсор `next_charge_date` сдвигается на `interval_months` в момент
> материализации. «Сегодня/просрочено» — из материализованных строк
> (`GET /subscriptions/recurring/due`), проведение в факт по подтверждению.

**Идемпотентность:**

- материализация регулярок: ключ `(subscription_id, original_charge_date)` уникален в `planned_transaction` → unique constraint предотвращает дубли при повторном запуске.
