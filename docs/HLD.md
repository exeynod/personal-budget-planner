...
| `close_period` | ежедневно 00:01 | Для активного периода, чей `period_end < today` — авто-пропуск (удаление) непроведённых `PlannedTransaction(subscription_auto)` закрываемого периода, затем закрыть (`status=closed`, `ending_balance` = расчёт). Создать новый активный период, развернуть шаблон + материализовать регулярные платежи, сдвигая `next_charge_date` на `interval_months`. |
