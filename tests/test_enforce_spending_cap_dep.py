...
def _remove_cap_test_route(app):
    app.router.routes = [
        r for r in app.router.routes if getattr(r, "path", None) != _CAP_ROUTE
    ]
...
async def test_over_cap_returns_429_with_retry_after(db_client, bot_token, owner_tg_id):
    ...
    try:
        ...
        assert retry_after.isdigit(), (
            f"Retry-After must be digit string, got {retry_after!r}"
        )
        ...
        assert detail.get("spent_cents") == 250, (
            f"spent_cents expected 250, got {detail}"
        )
...
# NOTE (prune): test_cap_zero_blocks_with_logs (dup of cap_zero_blocks_immediately)
# and test_member_with_own_cap_under_limit (per-user cap isolation already
# covered by tests/test_spend_cap_service.py::test_spend_cents_isolated_per_user
# + the integration suite) were removed.
